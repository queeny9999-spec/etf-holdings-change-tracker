...
const rawDate = '20260924';
const shortDate = '09/24';
const weekday = '四';
...
function parseXiaoyuAmounts() {
  const raw = fs.readFileSync(`_work/data/xiaoyu-data-${rawDate}.js`, 'utf8')
    .replace(/^window\.DATA\s*=\s*/, '')
    .replace(/;\s*$/s, '');
  const data = JSON.parse(raw);
  if (data.meta?.latest !== rawDate && data.meta?.latest_ymd !== rawDate && data.meta?.latest_slash !== date) {
    throw new Error(`Unexpected Xiaoyu meta date: ${JSON.stringify(data.meta)}`);
  }
  const result = {};
  for (const code of ['00981A', '00403A', '00991A']) {
    const item = data.etfs.find((etf) => etf.code === code);
    if (!item || item.date !== rawDate) throw new Error(`Xiaoyu data missing for ${code}`);
    result[code] = {
      date,
      buy: item.buy_amt,
...
const fuhwa = JSON.parse(fs.readFileSync(`_work/data/ETF23-${rawDate}.json`, 'utf8'));
const official = {
  '00981A': parseEzmoney(`ezmoney-49YTW-check-${rawDate}.html`, '00981A', '主動統一台股增長'),
  '00403A': parseEzmoney(`ezmoney-63YTW-check-${rawDate}.html`, '00403A', '主動統一升級50'),
  '00991A': {
...
if (inst.slash !== date) throw new Error(`Xiaoyu inst report not ready: ${inst.slash}`);
...
for (const target of targets) {
  ...
  for (const code of Object.keys(official)) {
    ...
    notes.value[code] = `${shortDate} 的持有股數與比重來自${code === '00991A' ? '復華' : '統一'}投信官網，${shortDate} 欄位為較 09/23 官網快照的實際股數異動。`;
  }
  ...
  html = html.replace(/data\.js\?v=\d+/g, `data.js?v=${rawDate}`);
  html = html.replace(/inst\.json\?v=\d+/g, `inst.json?v=${rawDate}`);
